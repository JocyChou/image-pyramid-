...
Vector<Mat> pyramidb(Mat source, string savename, cv::Mat filter, int iw, int ih, int fw, int fh){
    int i = 0;
    Vector<Mat> pyrb(8);
    savename = "/Users/Jocy/Dropbox/2016 Fall/CSI 111/Program assignment/ProgramAssignment2/text/part1/text";
    string presavename = savename + "pyramidA0.jpg";
    //imwrite(presavename, source);
    for(i = 0; i < 8; i++){
        Mat preimage = convolve(source , filter, iw, ih, filter.cols, filter.rows);
        pyrb[i] = preimage;
        //string presavename = savename + "pyramidB" + to_string(i+1) + ".jpg";
        //imwrite(presavename, preimage);
        filter = Mat(2 * filter.cols, 2 * filter.cols, CV_32F, Scalar(1.0/(2 * filter.cols * 2 * filter.cols)));
    }
    return pyrb;
}
...
void Lappyramid(Mat source, string savename, cv::Mat filter, int iw, int ih, int fw, int fh){
    ...
    savename = "/Users/Jocy/Dropbox/2016 Fall/CSI 111/Program assignment/ProgramAssignment2.1/part3/text";
    for(i = 1; i < 8; i++){
        ...
        prelevel = (prelevel + (0 - pyramin))/((pyramax - pyramin)/(max - min));
...
void Highpassa(Mat source, string savename, cv::Mat filter, int iw, int ih, int fw, int fh){
    int i = 0;
    double min = 0;
    double max = 0;
    double* minp = &min;
    double* maxp = &max;
    minMaxIdx(source,minp,maxp);
    Vector<Mat> highpassa(8);
    Vector<Mat> gaupyramid = pyramida(source, savename, filter,source.cols, source.rows, filter.cols, filter.rows);
    savename = "/Users/Jocy/Dropbox/2016 Fall/CSI 111/Program assignment/ProgramAssignment2.1/part4/bloodvessels";
    for(i = 0; i < 8; i++){
        Mat resizecur = imresize(gaupyramid[i], gaupyramid[i].cols, gaupyramid[i].rows, iw, ih);
        Mat prelevel = source - resizecur;
        prelevel = prelevel + source;
        double pyramin = 0;
        double pyramax = 0;
        double* pyraminp = &pyramin;
        double* pyramaxp = &pyramax;
        minMaxIdx(prelevel, pyraminp, pyramaxp);
        prelevel = (prelevel + (0 - pyramin))/((pyramax - pyramin)/(max - min));
        highpassa[i] = prelevel;
        string presavename = savename + "gaupyramidhighpassA" + to_string(i) + ".jpg";
        imwrite(presavename, prelevel);
        //cvNamedWindow("Highpass:", CV_WINDOW_AUTOSIZE);
        //imshow("Highpass", prelevel);
    }
}

void Highpassb(Mat source, string savename, cv::Mat filter, int iw, int ih, int fw, int fh){
    int i = 0;
    Vector<Mat> highpassfilter(7);
    highpassfilter[0] =Mat(fw, fh, CV_32F, Scalar(0)) - filter;
    highpassfilter[0].at<float>(0, fw - 1) += 1;
    savename = "/Users/Jocy/Dropbox/2016 Fall/CSI 111/Program assignment/ProgramAssignment2/earth/part3/earth";
    for(i = 1; i < 7; i++){
        highpassfilter[i] = Mat(2 * highpassfilter[i-1].cols, 2 * highpassfilter[i-1].cols, CV_32F, Scalar(0)) - Mat(2 * highpassfilter[i-1].cols, 2 * highpassfilter[i-1].cols, CV_32F, Scalar(1.0/(2 * highpassfilter[i-1].cols * 2 * highpassfilter[i-1].cols)));
        highpassfilter[i].at<float>(0, 2 * highpassfilter[i-1].cols - 1) += 1;
    }
    Vector<Mat> highpassb(7);
    for(i = 0; i < 7; i++){
        Mat prelevel = convolve(source , highpassfilter[i], source.cols, source.rows, highpassfilter[i].cols, highpassfilter[i].rows);
        highpassb[i] = prelevel;
        string presavename = savename + "filterhighpassB" + to_string(i) + ".jpg";
        imwrite(presavename, prelevel);
        cvNamedWindow("Highpass:", CV_WINDOW_AUTOSIZE);
        imshow("Highpass", prelevel);
    }
